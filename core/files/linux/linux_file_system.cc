...
#include "core/files/linux/linux_file_system.h"
...
class LinuxRandomAccessFile : public RandomAccessFile {
 ...
 public:
  ...
  Status Read(uint64_t offset, size_t n, StringPiece* result,
...
class LinuxReadOnlyMemoryRegion : public ReadOnlyMemoryRegion {
 public:
  LinuxReadOnlyMemoryRegion(const void* address, uint64_t length)
...
Status LinuxFileSystem::CreateDir(const string& name) {
  Status result;
...
Status LinuxFileSystem::DeleteDir(const string& name) {
  Status result;
...
Status LinuxFileSystem::GetFileSize(const string& fname, 
		                    uint64_t* size) {
  Status s;
  struct stat sbuf;
